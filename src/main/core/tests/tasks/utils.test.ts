...
import { isSubtask } from '../../tasks/utils';
import type { Task } from '../../tasks/types';
import { TaskPriority, TaskStatus } from '../../tasks/types';
import { localDayWindow } from '@main/core/shared/utils';

describe('localDayWindow', () => {
  it('start is midnight at the beginning of the given day', () => {
    const { startOfDay: start } = localDayWindow(new Date(2024, 5, 15, 14, 30, 0));
  ...
  it('end is midnight at the beginning of the next day', () => {
    const { endOfDay: end } = localDayWindow(new Date(2024, 5, 15, 14, 30, 0));
  ...
  it('end is strictly after start', () => {
    const { startOfDay: start, endOfDay: end } = localDayWindow(new Date(2024, 5, 15, 9, 0, 0));
    expect(end.getTime()).toBeGreaterThan(start.getTime());
  });

  it('input at the very start of a day still maps to that day', () => {
    const midnight = new Date(2024, 5, 15, 0, 0, 0, 0);
    const { startOfDay: start } = localDayWindow(midnight);
    expect(start.getDate()).toBe(15);
  });

  it('input at 23:59:59.999 still maps to the same day', () => {
    const almostMidnight = new Date(2024, 5, 15, 23, 59, 59, 999);
    const { startOfDay: start, endOfDay: end } = localDayWindow(almostMidnight);
    expect(start.getDate()).toBe(15);
    expect(end.getDate()).toBe(16);
  });

  it('rolls over correctly at the last day of a month', () => {
    const { startOfDay: start, endOfDay: end } = localDayWindow(new Date(2024, 0, 31, 12, 0, 0)); // Jan 31
  ...
  it('rolls over correctly at the last day of the year', () => {
    const { startOfDay: start, endOfDay: end } = localDayWindow(new Date(2024, 11, 31, 12, 0, 0)); // Dec 31
    expect(start.getFullYear()).toBe(2024);
  ...
  it('handles leap day (Feb 29)', () => {
    const { startOfDay: start, endOfDay: end } = localDayWindow(new Date(2024, 1, 29, 12, 0, 0)); // Feb 29 2024
